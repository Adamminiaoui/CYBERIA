...
function App() {
  ...
  const scoreContext = useMemo(() => {
    ...
    const consensus =
      consensusPct >= 60
        ? 'High cross-feed consensus'
        : consensusPct >= 30
          ? 'Moderate cross-feed consensus'
          : 'Low cross-feed consensus';
    ...
    let explanation =
      'Score and feed count are aligned with the available provider evidence.';
    if (result.score >= 80 && flagged <= 1) {
      explanation =
        'A single provider returned very strong malicious detections. Score is high due to severity, even with limited feed consensus.';
    } else if (result.score <= 30 && flagged >= 3) {
      explanation =
        'Several providers flagged weak or ambiguous signals, so consensus is broad but severity remains low.';
    }
          ...
          <section className={`results ${submitted ? 'visible' : ''}`} aria-live="polite">
            ...
            {scoreContext && (
              <article className="risk-context" aria-label="Risk score interpretation">
                ...
                <ul>
                  <li>{scoreContext.severity}</li>
                  <li>{scoreContext.consensus}</li>
                  <li>{scoreContext.explanation}</li>
                  <li>
                    Active sources:{' '}
