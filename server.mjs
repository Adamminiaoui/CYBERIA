...
function createAnalysisResponse({
  ...
  const score =
    typeof scoreOverride === 'number'
      ? normalizeScore(scoreOverride)
      : normalizeScore(vendor.malicious * 12 + vendor.suspicious * 6 + vendor.confidenceBoost);
  return {
    score,
    category: categoryOverride || computeCategory(vendor),
...
async function vtSubmitUrlAndGetReport(url) {
  if (!vtKey) return null;
  ...
  if (!submitRes.ok) {
    console.warn(`VirusTotal URL submit failed with status ${submitRes.status}`);
    return vtGetUrlReport(url);
  }
...
async function vtUploadAndPoll(file) {
  ...
  if (!uploadRes.ok) {
    console.warn(`VirusTotal upload failed with status ${uploadRes.status}`);
    return null;
  }
...
function stripAbuseChComments(text) {
  const lines = String(text || '').split(/\r?\n/);
  const output = [];
  for (const line of lines) {
    const trimmedLeft = line.replace(/^\uFEFF?/, '').trimStart();
    if (!trimmedLeft) continue;
    if (trimmedLeft.startsWith('#')) {
      const candidate = trimmedLeft.slice(1).trimStart();
      // Some abuse.ch feeds comment the header: "# col1,col2,..."
...
async function importUrlhausAndFeodo({ mongoUri }) {
  const [urlhausRes, feodoRes] = await Promise.all([
    fetch('https://urlhaus.abuse.ch/downloads/csv_recent/'),
    fetch('https://feodotracker.abuse.ch/downloads/ipblocklist.csv')
  ]);

  if (!urlhausRes.ok) {
    throw new Error(`URLhaus download failed (${urlhausRes.status})`);
  }
  if (!feodoRes.ok) {
    throw new Error(`Feodo download failed (${feodoRes.status})`);
  }

  const urlhausText = stripAbuseChComments(await urlhausRes.text());
  const feodoText = stripAbuseChComments(await feodoRes.text());

  const urlhausRows = parseCsv(urlhausText, {
    columns: true,
    skip_empty_lines: true
  });
  const feodoRows = parseCsv(feodoText, {
    columns: true,
    skip_empty_lines: true
  });
  ...
  try {
    const db = client.db('drift_db');
    const urls = db.collection('malicious_urls');
    const ips = db.collection('malicious_ips');

    const urlOps = urlhausRows
      .filter((row) => row?.url)
      .map((row) => ({
        updateOne: {
          filter: { url: row.url },
          update: {
            $set: {
              ...
              urlhaus_link: row.urlhaus_link ?? null,
    ...
    const ipOps = feodoRows
      .filter((row) => row?.dst_ip)
      .map((row) => ({
        updateOne: {
          filter: { ip: row.dst_ip },
          update: {
            $set: {
              source: 'feodo_tracker',
              ingested_at: new Date().toISOString(),
              ip: row.dst_ip,
              dst_port: row.dst_port ?? null,
              c2_status: row.c2_status ?? null,
              first_seen_utc: row.first_seen_utc ?? null,
              last_online: row.last_online ?? null,
              malware_family: row.malware ?? null,
              raw: row
            }
          },
          upsert: true
    ...
    const [urlResult, ipResult] = await Promise.all([
      urlOps.length ? urls.bulkWrite(urlOps, { ordered: false }) : Promise.resolve(null),
      ipOps.length ? ips.bulkWrite(ipOps, { ordered: false }) : Promise.resolve(null)
    ]);

    return {
      urlhaus: {
        rowsParsed: urlhausRows.length,
        upserted: urlResult?.upsertedCount ?? 0,
        modified: urlResult?.modifiedCount ?? 0
      },
      feodo: {
        rowsParsed: feodoRows.length,
        upserted: ipResult?.upsertedCount ?? 0,
        modified: ipResult?.modifiedCount ?? 0
      }
    };
  } finally {
    await client.close();
...
app.post('/api/full-drive/import', async (_req, res) => {
  try {
    const result = await importUrlhausAndFeodo({ mongoUri: MONGO_URI });
    return res.json({ ok: true, mongoUri: MONGO_URI, ...result });
  } catch (error) {
  ...
  try {
    ...
    if (mode === 'email') {
      ...
      [vtData, phishData, otxData] = await Promise.all([
        vtSubmitUrlAndGetReport(url),
        phishTankCheckUrl(url),
        otxGeneral('domain', domain)
      ]);
    }
    ...
    const noProviderData = !vtData && !abuseData && !otxData && !phishData;
    ...
    const responsePayload = createAnalysisResponse({
      ...
      firstSeen: vtData?.data?.attributes?.first_submission_date
        ...
        : 'Unknown',
      mitreTechnique: noProviderData ? 'Provider data unavailable' : computeMitreFromTags(tags),
      summary: noProviderData
        ? 'No external provider returned data for this target at this time. This is usually caused by API rate limits, temporary provider outages, or unsupported indicator types. Verify API quotas and try again shortly.'
        : summaryParts.join(' '),
      categoryOverride: noProviderData ? 'Provider Unavailable' : undefined,
      scoreOverride: noProviderData ? 0 : undefined,
      sources: {
...
app.post('/api/analyze/file', upload.single('file'), async (req, res) => {
  try {
    ...
    const noProviderData = !vtAnalysis && !bazaar;
    ...
    return res.json(
      createAnalysisResponse({
        ...
        mitreTechnique: noProviderData ? 'Provider data unavailable' : computeMitreFromTags(bazaarTags),
        summary: noProviderData
          ? 'No file intelligence provider returned data right now. Your key can still be valid; common causes are quota/rate limits or temporary provider/network issues. Try a smaller file or retry in a few minutes.'
          : narrative,
        categoryOverride: noProviderData ? 'Provider Unavailable' : undefined,
        scoreOverride: noProviderData ? 0 : undefined,
        sources: {
        ...
        metrics: {
          ...
          malwareBazaar: {
            matchCount: bazaarFound ? bazaar.data.length : 0,
            tagCount: bazaarTags.length
          }
        }
      })
    );
